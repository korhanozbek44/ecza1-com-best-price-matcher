// ==UserScript==
// @name         Ecza1 En Ucuz Combinasyon Bulucu
// @namespace    http://tampermonkey.net/
// @version      2024-08-10
// @description  En ucuz sepet oluşturucu
// @author       Korhan Özbek
// @match        https://www.ecza1.com/Shop/*
// @icon         https://www.google.com/s2/favicons?sz=64&domain=ecza1.com
// @grant        none
// ==/UserScript==
...
(function () {
  ...
  const getProducts = () => {
    const savedProducts = localStorage.getItem(
      'corehiProductCombinationFinder',
    );
  ...
  const addProduct = ({target}) => {
    ...
    const productTitle = window.location.pathname.includes('UserProductDetail')
      ? document.title
      : document.querySelector('.product-item__title').innerText;
    if (window.location.pathname.includes('UserProductDetail')) {
      const url = document.querySelector(
        '#content > div.container.product-detail > div.mb-2 > div > div.col-md-12.col-lg-3.col-xl-3 > div > div > div.font-size-14.pb-2.border-color-1.border-bottom.mb-3 a',
      ).href;
    ...
    getStoreListWrapper()
      ...
      .forEach(wrapper => {
        ...
        const id = window.location.pathname.includes('UserProductDetail')
          ? url.match(/.*UserProductDetail\?id=(\d+)/)[1]
          : url.match(/.*UserProfile\?userId=(\d+)/)[1];
        const price = parseFloat(
          wrapper
            .querySelector('.all-price-item-content.price')
            .innerText.replace('\n', '')
            .replace(' TL', '')
            .replace(',', '.'),
        );
  ...
  const writeCombinationOutput = combinationResults => {
    ...
    combinationResults.forEach(r => {
      ...
      resultItemContainer.innerHTML = `${matchedProductsMap.map(p => p.text).join('<br><br>')}<br><br>Toplam: ${matchedProductsMap.map(p => p.foundedPrice).reduce((previous, current) => {
        previous += current;
        return previous;
      }, 0)} TL`;
    ...
    products.forEach(p => {
      const match = products.filter(
        _p =>
          ...
          p.storeID === _p.storeID &&
          !results.map(r => r.storeID).includes(_p.storeID),
      );
      if (match.length > 0) {
        results.push({
